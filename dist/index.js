...
(function($PS) {
  ...
  var closed = function (dictFoldable) {
      var $207 = Path.create(true);
      var $208 = Data_List.fromFoldable(dictFoldable);
      return function ($209) {
          return $207($208($209));
      };
  };                           
...
(function($PS) {
  ...
  var exports = $PS["LinearAlgebra.Vector"];
...
(function($PS) {
  ...
  var exports = $PS["LinearAlgebra.Matrix"];
  var Control_Applicative = $PS["Control.Applicative"];
  var Control_Bind = $PS["Control.Bind"];
  var Data_Array = $PS["Data.Array"];
  var Data_Boolean = $PS["Data.Boolean"];
  ...
  var multiply$prime = function (rx) {
      return function (cy) {
          return Control_Bind.bind(Control_Bind.bindArray)(rx)(function (r) {
              return Control_Bind.bind(Control_Bind.bindArray)(cy)(function (c) {
                  return Control_Applicative.pure(Control_Applicative.applicativeArray)(Data_Maybe.fromMaybe(0.0)(LinearAlgebra_Vector.dot(Data_Semiring.semiringNumber)(r)(c)));
              });
          });
      };
  };
...
(function($PS) {
  ...
  var vectorToPoint = function ($0) {
      return (function (arr) {
          return {
              x: Data_Maybe.fromMaybe(0)(Data_Array.index(arr)(0)),
              y: Data_Maybe.fromMaybe(0)(Data_Array.index(arr)(1))
          };
      })(LinearAlgebra_Matrix.toVector($0));
  };
  exports["vectorToPoint"] = vectorToPoint;
  ...
  exports["incSpeed"] = $foreign.incSpeed;
})(PS);
(function($PS) {
  // Generated by purs version 0.13.8
  "use strict";
  ...
  var transformation = function (key) {
      if (key === "ArrowLeft") {
          return rotateY(Effect_Unsafe.unsafePerformEffect(Utils.incSpeed));
      };
      if (key === "ArrowRight") {
          return rotateY(-Effect_Unsafe.unsafePerformEffect(Utils.incSpeed));
      };
      if (key === "ArrowUp") {
          return rotateX(Effect_Unsafe.unsafePerformEffect(Utils.incSpeed));
      };
      if (key === "ArrowDown") {
          return rotateX(-Effect_Unsafe.unsafePerformEffect(Utils.incSpeed));
      };
  ...
  var identityMatrix_2_4 = function (v) {
      return Data_Maybe.maybe(LinearAlgebra_Matrix.zeros(2)(4))(Control_Category.identity(Control_Category.categoryFn))(LinearAlgebra_Matrix.fromArray(2)(4)([ v, 0.0, 0.0, 0.0, 0.0, v, 0.0, 0.0 ]));
  };
  var transform3dMatrixTo2dMatrix = identityMatrix_2_4(2.0);
  var changeOrigin = function (w) {
      return function (h) {
          return Data_Maybe.fromMaybe(LinearAlgebra_Matrix.zeros(2)(1))(LinearAlgebra_Matrix.fromArray(2)(1)([ w / 2.0, h / 2.0 ]));
      };
  };
  var projectOn2d = function (w) {
      return function (h) {
          var $1 = Data_Functor.map(Data_Functor.functorArray)(LinearAlgebra_Matrix.add(changeOrigin(w)(h)));
          var $2 = Data_Functor.map(Data_Functor.functorArray)(LinearAlgebra_Matrix.multiply(transform3dMatrixTo2dMatrix));
          return function ($3) {
              return $1($2($3));
          };
      };
  };
  exports["projectOn2d"] = projectOn2d;
...
(function($PS) {
  ...
  var edgeLineWidth = 0.5;
  var renderSide = (function () {
      var $0 = Graphics_Drawing.outlined(Data_Semigroup.append(Graphics_Drawing.semigroupOutlineStyle)(Graphics_Drawing.outlineColor(Color.black))(Graphics_Drawing.lineWidth(edgeLineWidth)));
      var $1 = Graphics_Drawing.closed(Data_Foldable.foldableArray);
      return function ($2) {
          return $0($1($2));
      };
  })();
  var drawEdges = function (points) {
      var to = function (edge) {
          return Utils.vectorToPoint(Data_Maybe.fromMaybe(LinearAlgebra_Matrix.zeros(2)(1))(Data_Array.index(points)(Data_Maybe.fromMaybe(0)(Data_Array.index(edge)(1)))));
      };
      var from = function (edge) {
          return Utils.vectorToPoint(Data_Maybe.fromMaybe(LinearAlgebra_Matrix.zeros(2)(1))(Data_Array.index(points)(Data_Maybe.fromMaybe(0)(Data_Array.index(edge)(0)))));
      };
      return Data_Functor.map(Data_Functor.functorArray)(function (edge) {
          return [ from(edge), to(edge) ];
      })(edges);
  };
  var renderCube = function (w) {
      return function (h) {
          return function (trans) {
              return function (cube) {
                  return Effect_Unsafe.unsafePerformEffect((function () {
                      var updated = Data_Functor.map(Data_Functor.functorArray)(LinearAlgebra_Matrix.multiply(trans))(cube);
                      return function __do() {
                          Utils.storeMatrix(updated)();
                          return Data_Foldable.foldMap(Data_Foldable.foldableArray)(Graphics_Drawing.monoidDrawing)(renderSide)(drawEdges(Vector.projectOn2d(w)(h)(updated)));
                      };
                  })());
              };
          };
  ...
  var cubeScale = 0.5;
  var cubeHalfExtent = 100.0;
  var point1 = [ -cubeHalfExtent, -cubeHalfExtent, cubeHalfExtent, 1 ];
  var point2 = [ cubeHalfExtent, -cubeHalfExtent, cubeHalfExtent, 1 ];
  var point3 = [ cubeHalfExtent, cubeHalfExtent, cubeHalfExtent, 1 ];
  var point4 = [ -cubeHalfExtent, cubeHalfExtent, cubeHalfExtent, 1 ];
  var point5 = [ -cubeHalfExtent, -cubeHalfExtent, -cubeHalfExtent, 1 ];
  var point6 = [ -cubeHalfExtent, cubeHalfExtent, -cubeHalfExtent, 1 ];
  var point7 = [ cubeHalfExtent, cubeHalfExtent, -cubeHalfExtent, 1 ];
  var point8 = [ cubeHalfExtent, -cubeHalfExtent, -cubeHalfExtent, 1 ];
  var cubePoints = Data_Functor.map(Data_Functor.functorArray)(LinearAlgebra_Vector.mulScalar(cubeScale))([ point1, point2, point3, point4, point5, point6, point7, point8 ]);
...
(function($PS) {
  ...
  var scene = function (keyboard) {
      return function (mouse) {
          return function (v) {
              var init = Effect_Unsafe.unsafePerformEffect(function __do() {
                  var cube = Utils.getCachedMatrix();
                  return Data_Foldable.foldMap(Data_Foldable.foldableArray)(Graphics_Drawing.monoidDrawing)(Cube.renderSide)(Cube.drawEdges(Vector.projectOn2d(v.w)(v.h)(cube)));
              });
              var background = Graphics_Drawing.filled(Graphics_Drawing.fillColor(Color.white))(Graphics_Drawing.rectangle(0.0)(0.0)(v.w)(v.h));
              var keyboardEvent = function (value) {
                  return function (v1) {
                      return Effect_Unsafe.unsafePerformEffect(function __do() {
                          var cube = Utils.getCachedMatrix();
                          return Data_Semigroup.append(Graphics_Drawing.semigroupDrawing)(background)(Cube.renderCube(v.w)(v.h)(Vector.transformation(value))(cube));
                      });
                  };
              };
              var mouseEvent = function (v1) {
                  if (v1 instanceof Data_Maybe.Just) {
                      return Effect_Unsafe.unsafePerformEffect(function __do() {
                          var oldX = Utils.getX();
                          var oldY = Utils.getY();
                          Utils.storeX(Data_Int.toNumber(v1.value0.x))();
                          Utils.storeY(Data_Int.toNumber(v1.value0.y))();
                          var cube = Utils.getCachedMatrix();
                          var ydeg = Data_Int.toNumber(v1.value0.y) - oldY;
                          var xdeg = Data_Int.toNumber(v1.value0.x) - oldX;
                          var trans = LinearAlgebra_Matrix.multiply(Vector.rotateX(-(ydeg * mouseSensitivity)))(Vector.rotateY(xdeg * mouseSensitivity));
                          return Data_Semigroup.append(Graphics_Drawing.semigroupDrawing)(background)(Cube.renderCube(v.w)(v.h)(trans)(cube));
                      });
                  };
                  if (v1 instanceof Data_Maybe.Nothing) {
                      return background;
                  };
                  throw new Error("Failed pattern match at Main (line 31, column 9 - line 31, column 55): " + [ v1.constructor.name ]);
              };
              return Data_Semigroup.append(FRP_Behavior.semigroupABehavior(FRP_Event.functorEvent)(Graphics_Drawing.semigroupDrawing))(FRP_Behavior.unfold(FRP_Event.eventIsEvent)(keyboardEvent)(FRP_Event_Keyboard.up)(init))(Data_Functor.map(FRP_Behavior.functorABehavior(FRP_Event.functorEvent))(mouseEvent)(FRP_Behavior_Mouse.position(mouse)));
          };
      };
  };
  var main = function __do() {
      ...
      if (mcanvas instanceof Data_Maybe.Nothing) {
          return Effect_Console.log("Main: canvas element with id 'canvas' not found; aborting init")();
      };
      if (mcanvas instanceof Data_Maybe.Just) {
          Utils.storeMatrix(Cube.initcube)();
          var ctx = Graphics_Canvas.getContext2D(mcanvas.value0)();
          var w = Graphics_Canvas.getCanvasWidth(mcanvas.value0)();
          var h = Graphics_Canvas.getCanvasHeight(mcanvas.value0)();
          Graphics_Canvas.setCanvasWidth(mcanvas.value0)(w)();
          Graphics_Canvas.setCanvasHeight(mcanvas.value0)(h)();
          var keyboard = FRP_Event_Keyboard.getKeyboard();
          var mouse = FRP_Event_Mouse.getMouse();
          FRP_Behavior.animate(scene(keyboard)(mouse)({
              w: w,
              h: h
          }))(Graphics_Drawing.render(ctx))();
          return Data_Unit.unit;
      };
      throw new Error("Failed pattern match at Main (line 50, column 3 - line 62, column 16): " + [ mcanvas.constructor.name ]);
  };
  exports["mouseSensitivity"] = mouseSensitivity;
  exports["scene"] = scene;
  exports["main"] = main;
})(PS);
PS["Main"].main();