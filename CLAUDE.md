...
- **Builder** — **slice 2**: an interactive **molecule-builder sandbox**. From a
  ...
  **continuous, frame-eased cross-fade** — even the `#zoom-in`/`#zoom-out` button
  ...
  in both layers, and the "Valence only" toggle composes with the LOD.
...
`element`, `view2D`, `subshellView`, `valenceOnly`, `bondProgress`, `zoom`, `detail`, `builder`) advanced each
