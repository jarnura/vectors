...
| `Update` | `Update.purs` | The `State` record definition + pure update folds: `initialState`, `applyDragStrength`, `applyValenceOnly`, `applySubshellView`, `applyOrbit`, `applyZoom`, `applyDetail`, `step`. Imported and re-exported by `Main` |
| `Scene.Entities` | `Scene/Entities.purs` | `Entity` / `EntityMesh` type definitions; per-scene entity list construction; `place` / LOD helper functions |
...
| `Scene` | `Scene.purs` | `Scene = CubePoc \| Atomos \| Molecule \| Builder \| Materials` (5 scenes), `nextScene` (5-cycles CubePoc→Atomos→Molecule→Builder→Materials→CubePoc), `sceneTitle`, atomos `spaceColor` |
